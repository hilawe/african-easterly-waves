...
def arrow(ax, xy0, xy1, color, lw, label=None, label_xy=None, ls="-", fs=9.0,
...
def panel(ax, active):
    ...
    ax.text(5.0, 10.25, title, fontsize=12.9, fontweight="bold", ha="center",
            color=CHAR)
    ...
    ax.add_patch(Rectangle((0.3, 0.9), 9.4, 3.6,
                           ...
                           ec="#c5ccd3", lw=0.8))
    ax.text(0.55, 8.85, "700 hPa  (jet level)", fontsize=9.5, color=CHAR, va="top")
    ax.text(0.55, 4.15, "850 hPa  (monsoon layer)", fontsize=9.5, color=CHAR,
            va="top")

    # trough axis through both layers
    trough_axis(ax, 6.6, 0.9, 9.2)
    ax.text(6.6, 9.55, "trough axis", fontsize=9.0, color=CHAR, ha="center")
    # muted W/E orientation ticks
    ax.text(0.35, 0.35, "W", fontsize=10.1, color=MUTED)
    ax.text(9.5, 0.35, "E", fontsize=10.1, color=MUTED, ha="right")
    ...
    if active:
        ...
        arrow(ax, (8.6, 9.0), (7.2, 7.9), SAHARA, 1.1,
              "reduced Saharan-side import", (8.15, 9.25), fs=8.7, ha="center")
        arrow(ax, (8.6, 5.9), (7.2, 6.9), MONSOON, 2.6,
              "more monsoon-side origins", (8.2, 5.75), fs=8.7, ha="center")
    else:
        arrow(ax, (8.6, 9.0), (7.2, 7.9), SAHARA, 3.0,
              "drier, warmer inflow", (8.15, 9.25), fs=8.7, ha="center")
        arrow(ax, (8.6, 5.9), (7.2, 6.9), MONSOON, 1.1,
              "fewer monsoon-side origins", (8.2, 5.75), fs=8.7, ha="center")
    ...
    if active:
        ...
        ax.text(cx, 6.35, "more MCS\nactivity", fontsize=8.7, color=CHAR,
    ...
    ax.text(1.25, 0.6, "shared SW monsoon inflow", fontsize=8.7, color=CHAR)
    if active:
        ...
        ax.text(5.35, 3.4, "moister (state contrast;\ntemperature and\ntheta-e not resolved)",
                fontsize=9.2, color="#00441b", ha="center", va="center", zorder=7)
        ...
        ax.text(8.35, 1.55, "prior-day convection;\nwake recovery (interpretation)",
                fontsize=8.1, color=CHAR, ha="center", va="top", style="italic",
                zorder=7)
    else:
    ...
    if active:
        ax.add_patch(FancyBboxPatch((1.15, 4.65), 4.3, 0.75,
                                    ...
                                    lw=0.9, zorder=8))
        ax.text(3.3, 5.02, "700 hPa: theta-e > 0, vapor over cooling", fontsize=9.2,
...
def main():
    ...
    fig.text(0.5, 0.015,
             ...
             fontsize=9.5, color=CHAR, ha="center")
